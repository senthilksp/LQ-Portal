...
<div class="contentWrapper_lq">
	<ul class="mainNav">
		<li class="level-1 here first" id="home"><a
			href='<%=LQ_HOME_URL%>'><%=rb.getString("header-portlet-Home")%></a>
		</li>
		...
		<c:if test="${roleName eq 'LEADER' || roleName eq 'LEADER_ADMIN'}">
			<li class="level-1" id="nav-sub-leadership"><a
				href='<%=LQ_QUEST_DETAILS_URL%>'><%=rb.getString("header-portlet-Quest")%></a></li>
		</c:if>
	...
	<div class="contentWrapper_lq">
		<c:if test="${(isSignedIn == true)}">
			<h6 style="float: right"><%=rb.getString("header-portlet-signin-caption")%>
				${LoggedUserBean.firstName} ${LoggedUserBean.lastName}
			</h6>
		</c:if>
	</div>
	<br> <br>
